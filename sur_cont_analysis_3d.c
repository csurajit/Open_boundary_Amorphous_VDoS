...
int main(){
    int i, j, k;
    unsigned int no_config = 41000;
    
    char parti_surf_FileName[128];
    FILE *parti_surf;
    sprintf(parti_surf_FileName,"parti_surf_of_%06u_ens_id_%d_particles_cut_out_dynamics_damping_annealed.txt",no_config,TP);
    parti_surf = fopen(parti_surf_FileName,"rb");
    double dummy1, dummy2, dummy3, dummy4;
    unsigned int load_configs=0, configs, dummy5;
    while ( fscanf(parti_surf,"%lf %lf %lf %lf %u",&dummy1,&dummy2,&dummy3,&dummy4,&dummy5) != EOF ){
        if(dummy4!=1.e0) load_configs++;
    }
    rewind(parti_surf);
    ...
    for(configs=0;configs<load_configs;configs++){
         fscanf(parti_surf,"%lf %lf %lf %lf %u",&(dummy1),&(participation[configs]),&(B2[configs]),&(pr_sur[configs]),&dummy5);
        if(pr_sur[configs]==1.e0) configs--;
    }
    fclose(parti_surf);
    ...
    sprintf(surface_contribution_distribution_FileName,"%d_surface_contribution_distribution_linear_%d_TP_annealed.txt",configs,TP);
