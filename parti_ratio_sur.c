...
int main(){
int f_up;
    unsigned int no_config = 500000, ens_id, load_config=0, config;
    ...
    double eigenvalue, participation_ratio,cmx,cmy;
    ...
    FILE *NQLM_File;    
    sprintf(NQLM_FileName,"parti_surf_of_%06u_ens_id_%d_particles_cut_out_dynamics_damping_new.txt",no_config,TP);
	   ...
	   for(i=0;i<TP;i++){
	    ...
	    double e2, e4, deno = 0.e0, nume = 0.e0, nume_sur = 0.e0; 
            for(k=0;k<TP;k++){
                    ...
                    dy=y[k]-cmy;
                    r2=((dx*dx)+(dy*dy));               
		        e2 = (delta_x[k]*delta_x[k])+(delta_y[k]*delta_y[k]);
		    	e4 = e2*e2;
		    	deno+=e4;
                	nume+=e2;
			//if(sqrt(r2)>6.5e0) nume_sur+=e2;
			if(sqrt(r2)>9.e0) nume_sur+=e2;
		    }
