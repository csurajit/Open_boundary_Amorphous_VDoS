...
int main(){
    ...
    for(config=0;config<no_config;config++){
        ...
        eigenvalueFile = fopen(eigenvalue_FileName,"r");
   ...
   //............... Data Files ....................
	...
	double *dos, *bin_width, *mid_omega;
	int *bin_config;
    ...
    for(i=0;i<no_of_binning;i++){
        dos[i]=dos[i]/(bin_width[i]*(load_config*(dimN-6)));      
        fprintf(density_of_states,"%E\t%E\t%d\n",mid_omega[i],dos[i],bin_config[i]);
    }
